# GrafanaMcpServer - Terraform Infrastructure

Infrastructure as Code for deploying the GrafanaMcpServer MCP server to Azure.
...
2. **Container image built and pushed**:
   ...
   grafana-mcp-server-cli docker push
   ```
...
container_image = "yourregistry.azurecr.io/grafana-mcp-server:latest"
...
grafana-mcp-server-cli infra init --env test \
...
terraform init \
  ...
  -backend-config="key=grafana-mcp-server-test.tfstate"
```
...
```bash
# Using CLI
grafana-mcp-server-cli infra validate --env test
...
```bash
# Using CLI
grafana-mcp-server-cli infra plan --env test \
  -i yourregistry.azurecr.io/grafana-mcp-server:latest \
  --state-rg <state-rg> \
...
```bash
# Using CLI (recommended)
grafana-mcp-server-cli infra apply --env test \
  -i yourregistry.azurecr.io/grafana-mcp-server:latest \
  --state-rg <state-rg> \
  --state-storage <state-storage>
...
```bash
# Using CLI
grafana-mcp-server-cli infra output --env test
...
# Deploy new version
grafana-mcp-server-cli infra apply --env test \
  -i yourregistry.azurecr.io/grafana-mcp-server:v1.2.0 \
  --state-rg <state-rg> \
  --state-storage <state-storage>
```
...
```bash
# Using CLI
grafana-mcp-server-cli infra destroy --env test \
  --state-rg <state-rg> \
  --state-storage <state-storage>
