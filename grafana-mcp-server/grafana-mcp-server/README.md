...
A AI agent based on the mcp-server template
...
grafana-mcp-server-cli docker build

# Or using Docker directly
docker build -t grafana-mcp-server:latest .

# Build with custom tag
docker build -t grafana-mcp-server:v1.0.0 .
```
...
```bash
# Using agenticai CLI
grafana-mcp-server-cli docker run

# Or using Docker directly
docker run -p 8000:8000 \
  --env-file .env \
  grafana-mcp-server:latest
...
```env
OTEL_ENABLED=true
OTEL_SERVICE_NAME=grafana-mcp-server
...
resource_group_name = "rg-grafana-mcp-server-dev"
location = "eastus"
environment = "dev"

# Container configuration
mcp_container_image = "your-registry.azurecr.io/grafana-mcp-server:latest"
...
jobs:
  deploy:
    ...
    steps:
      ...
      - name: Build and Push
        run: |
          grafana-mcp-server-cli docker build --tag ${{ github.sha }}
          grafana-mcp-server-cli docker push --tag ${{ github.sha }}
      
      - name: Deploy Infrastructure
        run: |
          grafana-mcp-server-cli iac deploy \
...
steps:
  - task: AzureCLI@2
    inputs:
      ...
      inlineScript: |
        grafana-mcp-server-cli docker build --tag $(Build.BuildId)
        grafana-mcp-server-cli docker push --tag $(Build.BuildId)
        grafana-mcp-server-cli iac deploy \
...
```bash
# Using CLI
grafana-mcp-server-cli docker build --tag latest
...
grafana-mcp-server-cli docker login --registry your-registry.azurecr.io

# Build and push
grafana-mcp-server-cli docker build --tag v1.0.0 --registry your-registry.azurecr.io
grafana-mcp-server-cli docker push --tag v1.0.0 --registry your-registry.azurecr.io
```
...
grafana-mcp-server-cli docker build --tag latest --registry your-registry.azurecr.io
grafana-mcp-server-cli docker push --tag latest --registry your-registry.azurecr.io

# 3. Deploy infrastructure
grafana-mcp-server-cli iac deploy \
  --subscription-id <your-subscription-id> \
  --resource-group rg-grafana-mcp-server-dev \
  --state-rg rg-terraform-state \
  --state-storage stterraformstate \
  --container-image your-registry.azurecr.io/grafana-mcp-server:latest
```
...
grafana-mcp-server-cli iac output

# Destroy infrastructure
grafana-mcp-server-cli iac destroy \
  --subscription-id <sub-id> \
  --state-rg <state-rg> \
  --state-storage <state-storage>
```
...
```bash
# Build image
grafana-mcp-server-cli docker build --tag <tag> --registry <registry>

# Push image
grafana-mcp-server-cli docker push --tag <tag> --registry <registry>

# Login to registry
grafana-mcp-server-cli docker login --registry <registry>
```
...
- GitHub Issues: [Report a bug](https://github.com/MiroslavKozma/grafana-mcp-server/issues)
...
**Author**: MiroslavKozma (miroslav.kozma.ext@bayer.com)  
**Version**: 0.1.0
