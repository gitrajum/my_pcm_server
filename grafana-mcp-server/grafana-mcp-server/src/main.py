...
# Import services
from .services.hello_world_service import register_hello_world_tools
from .services.grafana_service import register_grafana_tools
...
async def lifespan(mcp: FastMCP):
    ...
    logger.info(f"Starting GrafanaMcpServer MCP Server v0.1.0")
    ...
    logger.info("Shutting down GrafanaMcpServer MCP Server")
...
mcp = FastMCP(
    ...
    instructions="A AI agent based on the mcp-server template",
    version="0.1.0",
...
def main():
    ...
    try:
        ...
        logger.info(f"Starting MCP server in streamable-http mode on {settings.mcp_server_host}:{settings.mcp_server_port}")
